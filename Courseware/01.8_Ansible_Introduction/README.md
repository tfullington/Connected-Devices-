...
## Configuring your Ansible Hosts file

In order to manage a group of remote servers ("hosts") you need to list them for Ansible.  There are several ways to do this.  We will use the Ansible ```hosts``` file, an [Inventory File](http://docs.ansible.com/ansible/intro_inventory.html) in Ansible terminology.

In the ```connected-devices``` git repo, you will find an ```ansible``` directory.  

### Copy the hosts template file
...
For Ansible's default behavior to work, the ```hosts``` must be in the ```ansible``` directory and named ```hosts```.
...
# replace <PASSWORD> with the password of your LAMPi
#   (e.g., "ansible_ssh_pass=raspberry")
lampi ansible_user=pi ansible_ssh_host=<IP> ansible_ssh_pass=<PASSWORD>
...
For now, we only have one host defined, ```lampi```.  There are optional parameters after the host name, including the user name to use, the IP address, and password.

Please replace the ```<IP>``` with the IP address of your LAMPI and the ```<PASSWORD>``` with the password for the ```pi``` user (it had better not be the default!).

**NOTE:** Ansible typically uses PKI for SSH (public/private keys), but for simplicity, we are going to stick with passwords for the moment.
...
2. Connect via the [Serial Port](../01.2_Assemble_LAMPI_and_Serial_Connection/README.md)
3. Configure [Wifi](../01.3_Set_Up_Wifi/README.md)
4. Update your ```ansible/hosts``` file if necessary (e.g., if your IP address changed)
