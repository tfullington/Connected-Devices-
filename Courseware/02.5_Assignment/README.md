...
* Controls the brightness of the lamp via PWM
* Toggles the lamp on and off
* Shows the current color of the lamp
* Starts up automatically when the lamp is powered on using cron.d

The app should look similar to the following:
...
* Your LED control code should be contained in a single Python file and class.  A minimal ```led_driver.py``` has been provided for you.
...
* When you create a cron job for your app, don't forget they run out of **cron.d** in alphanumeric order, and that pigpiod needs to run first. A common convention is to prefix a number to the filenames in the order they need to be run.
